The hybrid architecture ensures that your production LLM traffic continues flowing even during extended Management Plane outages, with automatic recovery once connectivity is restored.

## Data Plane to Management Plane Communication

### Network Flow Diagram

```
## Data Plane ↔ Management Plane Communication Flow
...
└─────────────┬─────────────────────────────────┬─────────────────────────┘
    ...
    └─────────┬─────────────┘         └─────────┬────────────────┘
              ...
              └─────────────────┬───────────────┘
                                ...
                                │
              ┌─────────────────▼──────────────────────────────────────┐
              │        AIRS_GW MANAGEMENT PLANE (AIRS Gateway VPC)             │
              │              api.portkey.ai:443                        │
              │                                                        │
              │  ┌──────────────────────────────────────────────────┐  │
...
└──────┬─────────────────┘
       ...
       ▼
┌────────────────────────────────┐
│ HTTPS POST to Management Plane    │
│                                │
│ Headers:                       │
│ - Authorization                │
└──────┬─────────────────────────┘
...
## Management Plane Outage Scenario

```

Normal Operation
┌──────────────┐
│ Gateway      │◄──30s sync──► Management Plane ✓
│ Cached: OK   │
└──────────────┘

Management Plane Unreachable
┌──────────────┐
│ Gateway      │  ──sync──X     Management Plane ✗
...
│ • Management Plane UI operations                 │
└───────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────┐
│ Direct AIRS Gateway API Key in Request Headers           │
│                                                     │
│ curl -X POST <gateway-url>/v1/chat/completions \    │
│   -H "Authorization: Bearer sk-abc123" \            │
│   -H "x-airs-gw-provider: azure-openai" \           │
│   -H "x-airs-gw-api-key: <azure-key>" \             │
│   -d '{"model": "gpt-4", ...}'                      │
│                                                     │
└─────────────────────────────────────────────────────┘
⚠️ Bypasses provider config, but works during outage 

WHEN MANAGEMENT PLANE RECOVERS:
┌─────────────────────────────────────────────────────┐
...
```
┌─────────────────────────────────────────────────────┐
│ Data Security Maintained During Management Plane       │
│ Outage:                                             │
│                                                     │
...
│    • Encrypted at rest                              │
│    • Zero dependency on Management Plane               │
│                                                     │
│ ✅ API Keys & Secrets:                              │
...
│                                                     │
│ ✅ Authentication:                                  │
│    • AIRS Gateway API keys validated from cache          │
...
## Best Practices for Management Plane Outage Resilience
...
- [AIRS Gateway Hybrid Architecture Documentation](https://portkey.ai/docs/self-hosting/hybrid-deployments/architecture)
