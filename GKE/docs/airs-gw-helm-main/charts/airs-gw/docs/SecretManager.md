### AIRS Gateway — AWS Secrets Manager Integration

AIRS Gateway supports integrating AWS Secrets Manager via the Secrets Store CSI Driver.
...
```json
{
  ...
  "Statement": [
    {
      "Sid": "AIRS GatewaySecretsRead",
      "Effect": "Allow",
      "Action": [
        "secretsmanager:GetSecretValue",
        "secretsmanager:DescribeSecret"
      ],
      "Resource": "arn:aws:secretsmanager:<REGION>:<ACCOUNT_ID>:secret:airs-gw/*"
    }
  ]
}
```
...
Create an AWS Secrets Manager secret like `arn:aws:secretsmanager:<REGION>:<ACCOUNT_ID>:secret:airs-gw/gateway/config`:
```json
{
  "PORTKEY_CLIENT_AUTH": "<shared by airs-gw>",
...
metadata:
  name: airs-gw-aws-secrets
  namespace: airs-gw
spec:
  provider: aws
  parameters:
    objects: |
      - objectName: "arn:aws:secretsmanager:<REGION>:<ACCOUNT_ID>:secret:airs-gw/gateway/config"
...
# values.yaml
volumes:
  - name: airs-gw-secrets-store
    csi:
      driver: secrets-store.csi.k8s.io
      readOnly: true
      volumeAttributes:
        secretProviderClass: "airs-gw-aws-secrets"
volumeMounts:
  - name: airs-gw-secrets-store
...
- SecretProviderClass:
```bash
kubectl -n airs-gw get secretproviderclass airs-gw-aws-secrets -o yaml
```
- Secret exists after pod starts (Option A/C):
```bash
kubectl -n airs-gw get secret airs-gw-env -o yaml
```
- Mount present on pods:
```bash
kubectl -n airs-gw get pods -l app.kubernetes.io/name=airs-gw
kubectl -n airs-gw exec -it deploy/<release>-airs-gw -- ls -l /mnt/secrets
```
