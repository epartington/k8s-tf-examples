...
With Portkey, you can take advantage of features like fast AI gateway access, observability, prompt management, and more, all while ensuring the secure management of your Vertex auth through [Model Catalog](/docs/product/model-catalog).
...
<Note>
  **Self-Hosted GKE Deployments:** When running the Portkey Gateway on GKE with [Workload Identity Federation](/docs/self-hosting/hybrid-deployments/gcp#setting-up-iam-permission) enabled (`GCP_AUTH_MODE=workload`), the Gateway automatically acquires Vertex AI access tokens from the GCP metadata server. No manual `Authorization` header is needed.

  The GSA bound to the Gateway's KSA must have the `roles/aiplatform.user` role. See the [GCP deployment guide](/docs/self-hosting/hybrid-deployments/gcp#setting-up-iam-permission) for setup.
</Note>
...
<Card title="Counting Tokens" href="/docs/integrations/llms/vertex-ai/count-tokens">
...
Note that you will have to set [`strict_open_ai_compliance=False`](/docs/product/ai-gateway/strict-open-ai-compliance) in the headers to use this feature.
...
[Image Generation API Reference](/docs/api-reference/inference-api/images/create-image)
...
<Warning>
  The recommended way to attribute costs and track usage is to use [metadata](/docs/product/observability/metadata) which allows your workflows to be vendor agnostic
</Warning>
...
If you require the Chain of Thought response along with the actual inference text, pass the [strict open ai compliance flag](/docs/product/ai-gateway/strict-open-ai-compliance) as `false` in the request.

If you want to get the inference text only, pass the [strict open ai compliance flag](/docs/product/ai-gateway/strict-open-ai-compliance) as `true` in the request.
...
The response includes `safetyRatings` in each choice object. Set [`strict-open-ai-compliance`](/docs/product/ai-gateway/strict-open-ai-compliance) to `false` to receive `safetyRatings` in the response.
...
<CardGroup cols={2}>
  ...
  <Card title="Add Metadata" icon="tag" href="/docs/product/observability/metadata">
    Add metadata to your Vertex AI requests
  </Card>

  <Card title="Gateway Configs" icon="sliders" href="/docs/product/ai-gateway/configs">
    Configure advanced gateway features
  </Card>

  <Card title="Request Tracing" icon="chart-line" href="/docs/product/observability/traces">
    Trace and monitor your Vertex AI requests
  </Card>

  <Card title="Setup Fallbacks" icon="shield" href="/docs/product/ai-gateway/fallbacks">
